...
class VerifyEmail extends StatefulWidget {
  ...
  const VerifyEmail({super.key, required this.otpHash, required this.userModel, this.email = "",required this.reload});
...
class _VerifyEmailState extends State<VerifyEmail> {
  ...
  _verifyOTP() {
    ...
    APIService.verifyOTP(email, _hashCode, _otpCode)
        .then((response) async {
  ...
  _updateUser()async{
    final SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    Services.updateEmail(widget.email).then((response){
      ...
      if(response=="success"){
        ...
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text("Email address was updated successfully"),
              showCloseIcon: true,
            )
        );
        setState(() {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text("Email address was not updated."),
              action: SnackBarAction(label: "Try again", onPressed: _updateUser),
              showCloseIcon: true,
            )
        );
      } else if (response=="Error"){
        ...
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text("Email address already exists. Please try a different email address"),
              showCloseIcon: true,
            )
        );
      } else {
